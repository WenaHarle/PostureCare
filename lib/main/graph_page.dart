...
class _GraphPageState extends State<GraphPage> {
  ...
  // Replace with your provided epoch times
  final List<int> daysEpoch = [
    1721054443, 1721140843, 1721227243, 1721314093, 1721400493, 1721486893, 1721573293
  ];
  ...
  Future<void> _fetchData() async {
    try {
      for (int i = 0; i < daysEpoch.length; i++) {
        print('Fetching data for epoch time: ${daysEpoch[i]}');  // Debug log
        DatabaseEvent event = await _databaseReference.child('Data/${daysEpoch[i]}').once();
        ...
        if (snapshot.exists && snapshot.value != null) {
          ...
          if (data.containsKey('SensorCervical')) {
            ...
            await _databaseReference.child('WeeklyData/${daysEpoch[i]}').set(data);
          } else {
            setState(() {
